...
function triggerPulse(isSuper) {
    ...
    if (now - lastTime >= cooldown) {
        ...
        enemies = enemies.filter(en => {
            ...
            return !(dist < range && (!en.isBoss || isSuper));
        });
        flashColor = isSuper ? "#ffff00" : "#00f2ff";
        flashTimer = 30; isSuperActive = isSuper;
...
function update() {
    if (!gameActive || isPaused) return;
    let now = Date.now();
    if (now - lastScoreUpdate >= 1000) { score += 1; lastScoreUpdate = now; }
    ...
    enemies.forEach(en => {
        ...
        if (dist < player.size * 0.8 + en.size) gameOver();
    });
...
function draw() {
    ...
    if (flashTimer > 0) {
        ...
        ctx.beginPath(); ctx.arc(player.x, player.y, (isSuperActive ? 600 : 320) * (1 - flashTimer/30), 0, Math.PI*2); ctx.stroke();
        flashTimer--;
    }
    enemies.forEach(en => {
        ctx.beginPath(); ctx.arc(en.x, en.y, en.size, 0, Math.PI*2);
        ctx.fillStyle = en.isBoss ? "#bc13fe" : "#ff0000"; ctx.fill();
    });
    ctx.save(); ctx.translate(player.x, player.y); ctx.rotate(player.angle);
    ctx.drawImage(rocketImg, -player.size, -player.size, player.size*2, player.size*2); ctx.restore();
}
...
async function logActivity(action) {
    ...
    } catch (e) { console.log("Logger offline."); }
}
